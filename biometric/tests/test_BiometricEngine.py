...
class TestEdgeCases:
    ...
    def test_7_3_all_zero_channel(self):
        eng = _make_engine()
        raw = {
            "hr":   [0.0] * 10,
            "hrv":  [0.0] * 10,
...
class TestFatigueIndex:
    ...
    def test_9_4_fatigue_index_finite(self):
        fi = _make_engine().process(_make_valid_input(n=20))["metrics"]["fatigue_index"]
    ...
    def test_9_7_higher_load_raises_fatigue(self):
        ...
        assert fi_high > fi_low, \
            f"fi_high={fi_high:.6f} should exceed fi_low={fi_low:.6f}"
    ...
    def test_9_12_fatigue_result_has_value_and_raw(self):
        ...
        assert hasattr(fatigue, "value") and hasattr(fatigue, "raw")
...
class TestDriftScore:
    ...
    Key facts:
        ...
        All-constant channels → fatigue=0.5, hr_std=0, hrv_std=0 → drift=0.0
    ...
    def test_10_9_clamp_enforced_for_large_hr_std(self):
        ...
        ds = eng.process(raw)["metrics"]["drift_score"]
        assert ds <= 1.0
        assert ds == pytest.approx(1.0, abs=1e-5)
    ...
    def test_10_10_formula_cross_check(self):
        ...
        assert result["metrics"]["drift_score"] == pytest.approx(expected, abs=1e-5)
...
class TestInjuryRisk:
    ...
    Key facts:
        ...
        All-constant channels (load=100):
            ...
            risk = 0.45*0.5 + 0.35*0.0 + 0.20*0.1 = 0.245
    """
    ...
    def test_11_8_higher_load_raises_risk(self):
        """RW_LOAD is positive: higher normalised load → higher injury risk."""
        eng = _make_engine()
        low_load  = {"hr": self._c(70.0), "hrv": self._c(40.0),
    ...
    def test_11_9_load_signal_clamped_at_1000(self):
        """load_mean >= 1000 → load_signal = 1.0; injury_risk identical."""
        eng = _make_engine()
        raw_2k  = {"hr": self._c(70.0), "hrv": self._c(40.0),
                   "load": self._c(2000.0)}
        raw_1k  = {"hr": self._c(70.0), "hrv": self._c(40.0),
                   "load": self._c(1000.0)}
        ir_2k = eng.process(raw_2k)["metrics"]["injury_risk"]
        ir_1k = eng.process(raw_1k)["metrics"]["injury_risk"]
        assert ir_2k == pytest.approx(ir_1k, abs=1e-5), \
            f"Both load>=1000 must give same risk; got {ir_2k:.6f} vs {ir_1k:.6f}"
    ...
    def test_11_10_formula_cross_check(self):
        ...
        assert result["metrics"]["injury_risk"] == pytest.approx(expected, abs=1e-5)
...
class TestAnomalySignals:
    ...
    def test_12_7_constant_channels_anchor(self):
        ...
        eng, metrics, fatigue, drift = self._pipeline(raw)
        sig = eng.compute_anomaly_signals(metrics, fatigue, drift)
        assert sig.fatigue_signal == pytest.approx(0.5, abs=1e-5)
        assert sig.drift_signal   == pytest.approx(0.0, abs=1e-5)
        assert sig.hr_spread      == pytest.approx(0.0, abs=1e-5)
        assert sig.hrv_spread     == pytest.approx(0.0, abs=1e-5)
    ...
    def test_12_8_fatigue_signal_equals_fatigue_value(self):
        ...
        eng, metrics, fatigue, drift = self._pipeline(raw)
        sig = eng.compute_anomaly_signals(metrics, fatigue, drift)
        assert sig.fatigue_signal == pytest.approx(fatigue.value, abs=1e-9)
    ...
    def test_12_9_drift_signal_equals_drift_score(self):
        ...
        eng, metrics, fatigue, drift = self._pipeline(raw)
        sig = eng.compute_anomaly_signals(metrics, fatigue, drift)
        assert sig.drift_signal == pytest.approx(drift.score, abs=1e-9)
    ...
    def test_12_10_hr_spread_formula(self):
        ...
        assert sig.hr_spread == pytest.approx(expected, abs=1e-5)
    ...
    def test_12_11_hrv_spread_clamped_at_reference(self):
        ...
        sig = eng.compute_anomaly_signals(metrics, fatigue, drift)
        assert sig.hrv_spread == pytest.approx(1.0, abs=1e-5)
...
class TestAnomalyScoring:
    ...
    def test_13_8_formula_cross_check(self):
        ...
        assert result.score == pytest.approx(manual_score, abs=1e-5)
